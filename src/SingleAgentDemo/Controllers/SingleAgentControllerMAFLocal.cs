...
/// </summary>
[ApiController]
[Route("api/singleagent/maf_local")]  // Using constant AgentMetadata.FrameworkIdentifiers.MafLocal
...
Always call the tools in this order for best results.";
        ...
        _logger.LogInformation("SingleAgentControllerMAFLocal initialized with orchestrator agent and 4 MCP tools");
    }
        ...
        [FromForm] string customerId)
    {
        try
        {
            _logger.LogInformation(
                "Starting analysis for customer {CustomerId} using Single Agent with MCP Tools",
                customerId);
...
Provide a comprehensive analysis based on all tool results.";
            ...
            _logger.LogInformation("Executing orchestrator agent with 4 MCP tools");
            ...
            _logger.LogInformation("Analysis complete for customer {CustomerId} using Single Agent with MCP Tools", customerId);

            return Ok(analysisResponse);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in analysis for customer {CustomerId} using Single Agent with MCP Tools", customerId);
